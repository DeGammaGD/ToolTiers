...
public final class StandardReforgeRecipe extends AbstractTierifyAnvilRecipe {

    @Override
    public boolean matches(ItemStack left, ItemStack right, Player player) {
        return isStandardReforgeOperationValid(left, right);
    }
    ...
    public Optional<ItemStack> createResult(ItemStack left, ItemStack right, Player player) {
        ...
        var targetTier = pickResultTier(left, leftTier, rightTier);
        if (targetTier == null) {
            return Optional.empty();
        }
